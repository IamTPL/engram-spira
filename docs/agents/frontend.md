...
`app.css` (626 lines) is Tailwind v4 CSS-first — **there is no `tailwind.config.js/ts` anywhere.** Structure: `@import 'tailwindcss'` · `@custom-variant dark (&:where(.dark, .dark *))` · one `@theme` token block (L9-132) · a `.dark` override block (L135-185) · `@layer base` (L196-343) · `@layer utilities` (L345-462) · keyframes · a `prefers-reduced-motion` block (L613-626).
...
> `apps/web/src/app.css` is the **only** source of truth for design tokens. `docs/ui/design.md` is superseded: the June 2026 rewrite replaced the pastel/mint palette with a neutral shadcn/zinc scale (`--color-background` `#ffffff` / dark `#09090b`, `--color-primary` `#18181b` / dark `#fafafa`), changed every radius value, and moved buttons/inputs to `h-9` with `ring-1`. The CTA gradients were **recoloured, not removed** — `app.css:414` is commented "Legacy gradient utilities retained for current routes" and still defines `.btn-gradient` (415) and `.bg-section-gradient` (420) with `.dark` overrides at 188/192, still applied by login, register, not-found, study-mode, `flashcard/study-controls.tsx` and `focus/reward-popup.tsx`. Do not patch that doc's tables — regenerate from `app.css`.
>
> Vestigial: `app.css:113-114` still define `--sidebar-width: 16rem` and `--sidebar-collapsed-width: 3.5rem`. The only consumer is the `.w-sidebar` utility at `app.css:398`, which has **zero usages** in any `.tsx` — the app-shell explorer uses 296 px with 240–420 bounds from `app-shell-state.ts` instead.

`index.html` is also internally inconsistent with the redesign: its pre-JS loading shell hardcodes the retired palette and switches on `@media (prefers-color-scheme: dark)` while the app is class-based, so a user with a light OS and a dark app preference gets a light flash. It also hard-depends on Google Fonts (preconnect + preload swap for Inter).
