...
**`pgClient` does not behave like a fresh postgres.js client.** `drizzle(client)` mutates the client it is given: the postgres-js driver installs identity *parsers and serializers* for every timestamp/date/time OID and identity *serializers* for `json`/`jsonb`. Through `pgClient`, therefore: a bound `Date` throws inside postgres.js (`Received an instance of Date`), a bound object for a `$n::jsonb` placeholder throws (`Received an instance of Object`), and a `timestamptz` column is returned as text (`2026-01-11 12:00:00+00`). A test that opens its own `postgres()` client sees none of this — which is exactly how a 500 on `GET /study/deck/:id` shipped with a green suite. Always go through `src/db/pg-codecs.ts`: `bindTimestamp()`, `bindJson()` + `$n::text::jsonb` (the only jsonb form that round-trips on both kinds of client — the knowledge-graph repositories already use `::text::jsonb` for this reason), and `timestampFromRow()` on read. Postgres-backed tests must run the repository through a `drizzle()`-wrapped client too (`drizzleWrappedSql` in `fsrs-deck-reads.postgres.test.ts` / `fsrs-live.postgres.test.ts`).
