...
| GET | `/study/deck/:deckId/schedule` | — | `{totalCards, learnedCards, upcoming[], dueSoon, nextReviewDate}`; `learnedCards` counts mature cards: `state='review'` **and** `stability >= 21` (`LEARNED_STABILITY_DAYS`, `fsrs-deck-reads.postgres.ts`). Both the empty and non-empty paths return `dueSoon` |
...
| POST | `/study/review-batch` | `{items:[{requestId: uuid, cardId: uuid, rating, reviewedAt: date-time, durationMs?: 0..3_600_000}] 1..100}`, tz-aware | `{applied, duplicates, results:[{requestId, cardId, status:'applied'\|'duplicate', learningCycle, sequence, state, nextReviewAt, stability, difficulty, scheduledDays}]}` — one result per item, in request order. **409** for a reused `requestId` with a different card/rating; **422** for a duplicate `requestId` inside one batch or a `reviewedAt` more than 7 days before the server clock; **404** for a card the caller does not own. Client clocks are untrusted: a `reviewedAt` after the server's `receivedAt` is clamped to `receivedAt`, one earlier than the card's `last_reviewed_at` is clamped up to it (never rejected). Intervals are hard-capped at 365 days (`FSRS_MAX_INTERVAL_DAYS`) |
