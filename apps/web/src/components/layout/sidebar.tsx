...
const Sidebar: Component = () => {
  ...
  return (
    <aside class="w-64 border-r bg-card h-full overflow-y-auto flex flex-col">
                ...
                {(cls) => (
                  <div>
                    ...
                    <div class="flex items-center gap-1 group">
                      <Show
                        when={
                          renamingId() === cls.id && renamingType() === 'class'
                        }
                        fallback={
                          <button
                            ...
                            <div>
                              ...
                              <div class="flex items-center gap-1 group/folder">
                                <Show
                                  ...
                                  fallback={
                                    ...
                                    >
                                      <Show
                                        when={expandedFolders().has(folder.id)}
                                        fallback={
                                          <ChevronRight class="h-3 w-3 shrink-0 text-muted-foreground" />
                                        }
                                      >
                                ...
                                <Show when={renamingId() !== folder.id}>
                                  ...
                                  >
                                    ...
                                    <button
                                      class="h-6 w-6 flex items-center justify-center rounded text-destructive hover:bg-destructive/10 shrink-0"
                                      onClick={(e) =>
                                        handleDeleteFolder(e, cls.id, folder.id)
                                      }
                                    >
                                      <X class="h-3 w-3" />
                                    </button>
                              ...
                              <Show when={expandedFolders().has(folder.id)}>
                                <div class="ml-4 mt-0.5 space-y-0.5">
                                  {/* New Deck form */}
                                  <Show
                                    when={creatingDeckForFolder() === folder.id}
                                  >
                                    <form
                                      onSubmit={(e) =>
                                    ...
                                    >
                                      <Input
                                        ...
                                        value={newDeckName()}
                                        onInput={(e) =>
                                          setNewDeckName(e.currentTarget.value)
                                        }
                                        class="h-6 text-xs"
                                        autofocus
                                        onKeyDown={(e) => {
                                  ...
                                  {/* Decks */}
                                  <For each={decksByFolder()[folder.id] ?? []}>
                                    {(deck) => (
                                      <div class="flex items-center gap-1 group/deck">
                                        ...
                                        <Show when={renamingId() !== deck.id}>
                                          <Show
                                            when={confirmDeleteId() === deck.id}
                                            fallback={
                                              <>
                                                <button
